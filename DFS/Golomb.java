...
public class Golomb {
	...
	public void model() {
		...
		for (int i = 0; i < numbers.length; i++) {
			...
			numbers[i] = new IntVar(store, "n" + new Integer(i), (i + 1) * i
					/ 2, noMarks * noMarks);

			// Impose constraints that each consecutive number
		...
		for (int i = 1; i < noMarks; i++)
			for (int j = 0; j < i; j++)
				...
				store.impose(new XplusClteqZ(subs.get(index++), (noMarks - 1
						- i + j)
						* (noMarks - i + j) / 2, numbers[noMarks - 1]));
		...
		// FROMTHEINTERNET search = new FROMTHEINTERNET(store);
